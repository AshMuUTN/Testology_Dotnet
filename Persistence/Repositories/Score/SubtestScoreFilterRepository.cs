...
namespace Testology_Dotnet.Persistence.Repositories.Score
{
    public class SubtestScoreFilterRepository : BaseRepository, ISubtestScoreFilterRepository
    {
        ...
        {
            var subtestScoreFilter = await _context.SubtestScoreFilters.Where(ssf => ssf.Id == subtestScoreFilterId).FirstOrDefaultAsync();
        ...
        {
            return await _context.SubtestScoreFilters.Where(f => f.SubtestId == subtestId && f.DeletedAt == null).ToListAsync();
        }
